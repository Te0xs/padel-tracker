  ...
  const openModal = (match = null) => {
    if (match) {
      ...
      setFormData({
        ...
        set3: "",
        valorePartita: "",
        punteggioPre: lastPost.toFixed(2), // 2 decimali
      });
    }
    setIsModalOpen(true);
  };
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <form onSubmit={handleSubmit}>
              <input style={styles.input} type="date" value={formData.data} onChange={(e) => setFormData({ ...formData, data: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="A2" value={formData.giocatoreA2} onChange={(e) => setFormData({ ...formData, giocatoreA2: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="B1" value={formData.giocatoreB1} onChange={(e) => setFormData({ ...formData, giocatoreB1: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="B2" value={formData.giocatoreB2} onChange={(e) => setFormData({ ...formData, giocatoreB2: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="Set 1 (es: 6-4)" value={formData.set1} onChange={(e) => setFormData({ ...formData, set1: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="Set 2 (es: 3-6)" value={formData.set2} onChange={(e) => setFormData({ ...formData, set2: e.target.value })} required />
              <input style={styles.input} type="text" placeholder="Set 3 (opzionale)" value={formData.set3} onChange={(e) => setFormData({ ...formData, set3: e.target.value })} />
              <input style={styles.input} type="number" step="0.01" placeholder="Valore Partita" value={formData.valorePartita} onChange={(e) => setFormData({ ...formData, valorePartita: e.target.value })} required />
              <input style={styles.input} type="number" step="0.01" placeholder="Punteggio Pre" value={formData.punteggioPre} onChange={(e) => setFormData({ ...formData, punteggioPre: e.target.value })} required />
              <div style={{ marginTop: "10px" }}>
                <button style={styles.button} type="submit">Salva</button>
                <button style={styles.button} type="button" onClick={closeModal}>Annulla</button>
              </div>
            </form>
          </div>
        </div>
